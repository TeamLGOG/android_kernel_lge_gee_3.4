...
//const char* es325_fw_version = "M80.3.0.17_B17182_LGE_GK_Engineering_Release_MCFFillIn_STREAM";
const char* es325_dmic_flag = "dual mic"; // "dual mic" or "one mic"
...
enum {
	...
	ES325_MUSIC_PLAYBACK_ONE_CH,
	ES325_MUSIC_PLAYBACK,
	ES325_PRI_MIC_VOICE_REC,
	ES325_PRI_MIC_LOOPBACK,
	ES325_SEC_MIC_LOOPBACK,
	ES325_PRI_MIC_VIDEO_REC,
	ES325_PRI_MIC_VR,
	ES325_STOP
};
...
static u8 es325_internal_route_configs[ES325_STOP+1][60] = {
             { /* 1 - mic close talk - OPEN QUESTION [0] */
             0xb0, 0x5c, 0x00, 0x01, /* algo = voice processing */
             //0xb0, 0x4c, 0x00, 0x01, /* algo rate = 16kHz */
             //0xb0, 0x5e, 0x00, 0x01, /* mix rate = 16 kHz */
             0xb0, 0x5a, 0x28, 0xa0, // Set: SLIMRx0 -> UITONE1(APQ to 325)
             0xb0, 0x5a, 0x14, 0xa2, // Set: SLIMRx2 -> FEIN(MDM to 325)
             0xb0, 0x5a, 0x04, 0xa4, // Set: SLIMRx4 -> PRI(9310 to 325)
             0xb0, 0x5a, 0x40, 0xaa, // Set: SLIMTx0 <- CSOUT(325 to MDM)
             0xb0, 0x5a, 0x44, 0xae, // Set: SLIMTx4 <- FEOUT1(325 to 9310)
			 0x90, 0x31, 0x00, 0x00, /* Algo Preset */
             //0xb0, 0x17, 0x00, 0x02, /* mic config 1-MIC CT */
             //0xb0, 0x18, 0x00, 0x02, /* mic config 1-MIC CT */
             //0x90, 0x1c, 0x00, 0x00, /* algo processing = off */
             0xff                                   /* terminate */
             },
             { /* 2 - mic close talk [1] */
             //0xb0, 0x5c, 0x00, 0x01, // Set: Voice processing
             //0xb0, 0x4c, 0x00, 0x01, // Algo Processing rate = 16 kHz
             //0xb0, 0x5e, 0x00, 0x01, // Mix rate = 16 kHz
             //0xb0, 0x5a, 0x28, 0xa0, // Set: SLIMRx0 -> UITONE1(APQ to 325)
             //0xb0, 0x5a, 0x14, 0xa2, // Set: SLIMRx2 -> FEIN(MDM to 325)
             //0xb0, 0x5a, 0x04, 0xa4, // Set: SLIMRx4 -> PRI(9310 to 325)
             //0xb0, 0x5a, 0x08, 0xa5, // Set: SLIMRx5 -> SEC(9310 to 325)
             //0xb0, 0x5a, 0x40, 0xaa, // Set: SLIMTx0 <- CSOUT(325 to MDM)
             //0xb0, 0x5a, 0x44, 0xae, // Set: SLIMTx4 <- FEOUT1(325 to 9310)

			 0x90, 0x31, 0x00, 0x01, /* Algo Preset */

             //0xb0, 0x17, 0x00, 0x02, /* mic config 2-MIC CT */
             //0xb0, 0x18, 0x00, 0x00, /* mic config 2-MIC CT */
             //0x90, 0x1c, 0x00, 0x00, // Algo processing = off (COMMIT)
             0xff                                   // terminate
             },
             { /* 1 - mic desktop(far talk) [2] */
             0xb0, 0x5c, 0x00, 0x01, /* algo = voice processing */
             //0xb0, 0x4c, 0x00, 0x01, /* algo rate = 16kHz */
             //0xb0, 0x5e, 0x00, 0x01, /* mix rate = 16 kHz */
             0xb0, 0x5a, 0x28, 0xa0, // Set: SLIMRx0 -> UITONE1(APQ to 325)
             ...
             0xb0, 0x5a, 0x40, 0xaa, // Set: SLIMTx0 <- CSOUT(325 to MDM)
             0xb0, 0x5a, 0x44, 0xae, // Set: SLIMTx4 <- FEOUT1(325 to 9310)
             /* Original Parameters - doesn't work [Start] */
             //0xb0, 0x5a, 0x14, 0xa0, /* slim_rx0 -> FEIN */
             //0xb0, 0x5a, 0x44, 0xac, /* FEOUT -> slim_tx2 */
             //0xb0, 0x5a, 0x04, 0xa4, /* slim_rx4 -> PRI */
             //0xb0, 0x5a, 0x40, 0xaa, /* CSOUT -> slim_tx0 */
 			 /* Original Parameters - doesn't work [End] */

 			 0x90, 0x31, 0x00, 0x02, /* Algo Preset */

             //0xb0, 0x17, 0x00, 0x02,     /* mic config = 1-mic FT */
             //0xb0, 0x18, 0x00, 0x02,     /* mic config = 1-mic FT */
             //0x90, 0x1c, 0x00, 0x00, /* algo processing = off */
             0xff                                   /* terminate */
             },
             { /* 2 - mic far talk [3] */
			 ...
			 0x90, 0x31, 0x00, 0x03, /* Algo Preset */

             //0xb0, 0x17, 0x00, 0x02, /* mic config 2-MIC FT */
             //0xb0, 0x18, 0x00, 0x01, /* mic config 2-MIC FT */
             //0x90, 0x1c, 0x00, 0x00, // Algo processing = off (COMMIT)
             0xff                                   // terminate
             },
             { /* 1 - mic headset voice call [4] */
             0xb0, 0x5c, 0x00, 0x01, /* algo = voice processing */
             //0xb0, 0x4c, 0x00, 0x01, /* algo rate = 16kHz */
             //0xb0, 0x5e, 0x00, 0x01, /* mix rate = 16 kHz */
             0xb0, 0x5a, 0x28, 0xa0, // Set: SLIMRx0 -> UITONE1(APQ to 325)
             ...
             0xb0, 0x5a, 0x40, 0xaa, // Set: SLIMTx0 <- CSOUT(325 to MDM)
             0xb0, 0x5a, 0x44, 0xae, // Set: SLIMTx4 <- FEOUT1(325 to 9310)
             0xb0, 0x5a, 0x48, 0xaf, // Set: SLIMTx5 <- FEOUT2(325 to 9310)

			 0x90, 0x31, 0x00, 0x04, /* Algo Preset */

             //0x90, 0x1c, 0x00, 0x00, /* algo processing = off */
             0xff                                   /* terminate */
             },
             { /* music playback 1-channel [5] */
             0xb0, 0x5c, 0x00, 0x04, /* algo = 4 chan pass */
             //0xb0, 0x4c, 0x00, 0x01, /* algo rate = 16kHz */
             0xb0, 0x5a, 0x18, 0xa0, /* slim_rx0 -> AUDIN1 */
             0xb0, 0x5a, 0x4c, 0xac, /* AUDOUT1 -> slim_tx2 */

			 0x90, 0x31, 0x00, 0x05, /* Algo Preset */

             //0x90, 0x1c, 0x00, 0x00, /* algo processing = off */
             0xff                                   /* terminate */
             },
             { /* music playback 2-channel [6] */
             0xb0, 0x5c, 0x00, 0x04, /* algo = 4 chan pass */
             //0xb0, 0x4c, 0x00, 0x01, /* algo rate = 16kHz */
             0xb0, 0x5a, 0x18, 0xa0, /* slim_rx0 -> AUDIN1 */
             0xb0, 0x5a, 0x1c, 0xa1, /* slim_rx1 -> AUDIN2 */
             0xb0, 0x5a, 0x4c, 0xac, /* AUDOUT1 -> slim_tx2 */
             0xb0, 0x5a, 0x50, 0xad, /* AUDOUT2 -> slim_tx3 */

			 0x90, 0x31, 0x00, 0x06, /* Algo Preset */

             //0x90, 0x1c, 0x00, 0x00, /* algo processing = off */
             0xff                                   /* terminate */
             },
             { /* 1 - mic(pri) voice record [7] */
             0xb0, 0x5c, 0x00, 0x04, /* algo = 4 chan pass */
             //0xb0, 0x4c, 0x00, 0x01, /* algo rate = 16kHz */
             0xb0, 0x5a, 0x18, 0xa4, /* slim_rx4 -> AUDIN1 */
             0xb0, 0x5a, 0x4c, 0xaa, /* AUDOUT1 -> slim_tx0 */

			 0x90, 0x31, 0x00, 0x07, /* Algo Preset */

             //0x90, 0x1c, 0x00, 0x00, /* algo processing = off */
             0xff                                   /* terminate */
             },
             { /* loopback pri -> earpiece [8] */
             0xb0, 0x5c, 0x00, 0x04, /* algo = 4 chan pass */
             //0xb0, 0x4c, 0x00, 0x01, /* algo rate = 16kHz */
             //0xb0, 0x5e, 0x00, 0x01, /* mix rate = 16 kHz */
             0xb0, 0x5a, 0x18, 0xa0, /* slim_rx0 -> AUDIN1 */
             0xb0, 0x5a, 0x4c, 0xac, /* AUDOUT1 -> slim_tx2 */
             0xb0, 0x5a, 0x1c, 0xa4, /* slim_rx4 -> AUDIN2 */  // SPM: Was 0x18, should be 0x1c
             0xb0, 0x5a, 0x50, 0xaa, /* AUDOUT2 -> slim_tx0 */

			 0x90, 0x31, 0x00, 0x08, /* Algo Preset */

             //0x90, 0x1c, 0x00, 0x00, /* algo processing = off */
             0xff                                   /* terminate */
             },
             { /* loopback sec -> speaker [9] */
             0xb0, 0x5c, 0x00, 0x04, /* algo = 4 chan pass */
             //0xb0, 0x4c, 0x00, 0x01, /* algo rate = 16kHz */
             //0xb0, 0x5e, 0x00, 0x01, /* mix rate = 16 kHz */
             0xb0, 0x5a, 0x18, 0xa0, /* slim_rx0 -> AUDIN1 */
			 ...
			 0x90, 0x31, 0x00, 0x09, /* Algo Preset */
			 ...
			 0x90, 0x31, 0x00, 0x0a, /* Algo Preset */
			 ...
			 0x90, 0x31, 0x00, 0x0b, /* Algo Preset */

             //0x90, 0x1c, 0x00, 0x00, /* Algo processing = off (COMMIT) */
             0xff                                   /* terminate */
             },
             { /* clearing route for debugging [12] */
             0xb0, 0x4e, 0x09, 0xC4, /* bypass mute setup */
             0x90, 0x5c, 0x00, 0x00, /* stop route */
             0xff
             },
};
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	rc = es325_slim_write(es325, ES325_WRITE_VE_OFFSET,
							ES325_WRITE_VE_WIDTH, req_msg, 4, 1);
	if (rc < 0) {
		pr_err("%s(): failed write\n", __func__);
		return sprintf(buf, "status=0x0000FFFF\n");
	}
	...
	rc = es325_slim_read(es325, ES325_READ_VE_OFFSET,
							ES325_READ_VE_WIDTH, ack_msg, 4, 1);
	if (rc < 0) {
		pr_err("%s(): failed read\n", __func__);
		return sprintf(buf, "status=0xFFFF0000\n");
	}
	else {
		pr_info("%s(): ping ack = %02x%02x%02x%02x\n", __func__,
				ack_msg[0], ack_msg[1], ack_msg[2], ack_msg[3]);
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("%s():es325_internal_route_num = %ld\n",
				 ...
				 struct device_attribute *attr, char *buf)
{
		...
		(imsg & 0xFF))
	{
		...
		return sprintf(buf, "FW Version = Cannot Retrieve\n");
	    }
	    else
	    {
		mdelay(2);
	...
	return sprintf(buf, "FW Version = %s\n",verbuf);

}

static ssize_t es325_dmic_flag_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	pr_info("%s(): %s\n", __func__, es325_dmic_flag);

	return sprintf(buf, "%s\n" , es325_dmic_flag);
}

static ssize_t es325_debug_val_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	pr_info("%s(): debug_val=%d %d %d\n", __func__,
		es325_rx1_route_ena, es325_rx2_route_ena, es325_tx1_route_ena);
	return sprintf(buf, "debug_val=%d %d %d\n",
		es325_rx1_route_ena, es325_rx2_route_ena, es325_tx1_route_ena);
}

static ssize_t es325_debug_val_set(struct device *dev,
				struct device_attribute *attr, const char *buf, size_t count)
{
	long debug_val;
	int rc;
...
/*
	...
	/sys/devices/platform/msm_slim_ctrl.1/es325-codec-gen0/dmic_flag
	/sys/devices/platform/msm_slim_ctrl.1/es325-codec-gen0/es325_debug_val
*/
...
static struct device_attribute es325_device_attrs[] = {
	...
	__ATTR(dmic_flag,		S_IRUGO, \
			es325_dmic_flag_show,		NULL),
	__ATTR(debug_val,	S_IRUGO|S_IWUSR, \
			es325_debug_val_show,	es325_debug_val_set),
};
...
static int es325_bootup(struct es325_priv *es325)
{
	...
	pr_info("%s(): buf_frames = %d fw size=%d", __func__, buf_frames,es325->fw->size);
...
static int es325_defer_fw_download(struct es325_priv *priv)
{
	...
	if (!try_module_get(THIS_MODULE)) {
		pr_info("%s(): failed to increment module ref count\n", __func__);
		return -EFAULT;
	}
	task = kthread_run(fw_download, priv, "es325-firmware-dload");
	if (IS_ERR(task)) {
		module_put(THIS_MODULE);
		pr_info("%s(): failed to create kernel task\n", __func__);
		return PTR_ERR(task);
	}
	return 0;
}
...
int es325_codec_sleep(void)
{
	...
	rc = msm_xo_mode_vote(xo_handle_d1, MSM_XO_MODE_OFF);
...
int es325_codec_wakeup(void)
{
	...
	rc = msm_xo_mode_vote(xo_handle_d1, MSM_XO_MODE_ON);
	if (IS_ERR(xo_handle_d1)) {
		rc = PTR_ERR(xo_handle_d1);
		pr_err("%s: Failed to get the msm_xo_mode_vote(%d)\n", __func__, rc);
		goto EXIT_WAKEUP;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s(): route_num = %ld es325_internal_route_num = %ld\n",
			__func__, route_num, es325_internal_route_num);
...
static const char *es325_internal_route_configs_text[ES325_STOP+1] = {
	"One Mic CT",			"Two Mic CT",
	"One Mic DV",			"Two Mic FT",
	"One Mic Headset",
	"Music Playback One CH","Music Playback",
	"Pri Mic Voice Rec",
	"Pri Mic Loopback",		"Sec Mic Loopback",
	"Pri Mic Video Rec",
	"Pri Mic VR",
	"STOP"
};
...
static int es325_slim_probe(struct slim_device *sbdev)
{
	...
	if(strcmp(sbdev->name, "es325-codec-gen0") == 0) {
		...
		dev = device_create(es325_class, NULL, 0, NULL, "es325-codec");
		if (IS_ERR(dev))
			dev_err(&sbdev->dev, "%s(): failed to create device\n", __func__);
		for (cnt = 0; cnt < ARRAY_SIZE(es325_device_attrs); cnt++) {
			rc = device_create_file(dev, &es325_device_attrs[cnt]);
			if (rc){
				dev_err(&sbdev->dev, "%s(): failed to create class debug[%d]\n", __func__, cnt);
				device_remove_file(dev, &es325_device_attrs[cnt]);
			}
		}
	}
...
#ifndef CONFIG_SLIMBUS_DEVICE_UP_COND
	pr_info("%s(): deferring firmware download...\n", __func__);
...
static __init int es325_init(void)
{
...
#else
	xo_handle_d1 = msm_xo_get(MSM_XO_TCXO_A0, "es325");
	if (IS_ERR(xo_handle_d1)) {
		ret = PTR_ERR(xo_handle_d1);
		pr_err("%s: Failed to get the xo_handle_d1(%d)\n", __func__, ret);
	}
	else {
	...
	if (ret) {
		...
		ret = msm_xo_mode_vote(xo_handle_d1, MSM_XO_MODE_OFF);
	}
#endif
