...
#if defined(CONFIG_LGE_FELICA) || defined(CONFIG_LGE_NFC_SONY_CXD2235AGG) || defined(CONFIG_LGE_IRDA) || defined(CONFIG_LGE_IRRC)
...
static u8 mode7_red[]={0x40, 0x00, 0x35, 0xFE, 0x40, 0x5D, 0xE2, 0x00, 0x07, 0xAD, 0xE2, 0x00, 0x07, 0xAE, 0xE2, 0x00, 0x48, 0x00, 0x40, 0x5D, 0xE2, 0x00, 0x07, 0xAD, 0xE2, 0x00, 0x07, 0xAE, 0xE2, 0x00,};
static u8 mode7_green[]={0x40, 0x00, 0x35, 0xFE, 0x40, 0xCD, 0xE2, 0x00, 0x03, 0xE6, 0xE2, 0x00, 0x03, 0xE5, 0xE2, 0x00, 0x48, 0x00, 0x40, 0xCD, 0xE2, 0x00, 0x03, 0xE6, 0xE2, 0x00, 0x03, 0xE5, 0xE2, 0x00,};
static u8 mode7_blue[]={0x40, 0x00, 0x35, 0xFE, 0x40, 0xE6, 0xE0, 0x06, 0x03, 0xF2, 0xE0, 0x06, 0x03, 0xF2, 0xE0, 0x06, 0x48, 0x00, 0x40, 0xE6, 0xE0, 0x06, 0x03, 0xF2, 0xE0, 0x06, 0x03, 0xF2, 0xE0, 0x06,};

//[pattern_id 101, Felica_on]
static u8 mode8_blue[]={0x40, 0x33, 0x04, 0x65, 0x04, 0x65, 0x04, 0xE5, 0x04, 0xE5, 0x00, 0x00};
...
static int lp5521_setup(void)
{
	...
	if(lge_get_board_revno() == HW_REV_A)
	{
		...
		rc = gpio_request(LP5521_ENABLE_REV_A, "lp5521_led");

		if(rc){
			printk("LP5521: [%s] gpio_request(gpio #%d) failed!\n", __func__, LP5521_ENABLE_REV_A);
			return rc;
		}
		rc = gpio_tlmm_config(GPIO_CFG(LP5521_ENABLE_REV_A, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL , GPIO_CFG_2MA),GPIO_CFG_ENABLE);
...
static void lp5521_enable(bool state)
{
	...
	{
		if(state){
			gpio_set_value(LP5521_ENABLE_REV_A, 1);
			printk("LP5521: [%s] RGB_EN(gpio #%d) set to HIGH\n", __func__, LP5521_ENABLE_REV_A);
		}
		else{
			gpio_set_value(LP5521_ENABLE_REV_A, 0);
...
#define APQ_8064_RGB_LED_I2C_SLAVE_ADDR 0x32

static struct i2c_board_info lp5521_board_info[] __initdata = {
	{
		I2C_BOARD_INFO("lp5521", APQ_8064_RGB_LED_I2C_SLAVE_ADDR),
...
static struct msm_serial_hslite_platform_data apq8064_felica_gsbi3_pdata = {
	.config_gpio	= 1,
	.uart_tx_gpio	= 6,
	.uart_rx_gpio	= 7,
	.line		= 2,
...
static void __init apq8064_init_buses(void)
{
	...
	if(lge_get_board_revno() < HW_REV_C){
		gsbi_mem = ioremap_nocache(MSM_GSBI6_PHYS, 4);
...
static struct platform_device *felica_uart_gsbi3_devices[] __initdata = {
	&apq8064_device_felica_gsbi3,
};
#endif
...
#endif
#if defined(CONFIG_LGE_BROADCAST_ONESEG)
#define ONESEG_SPIS_XIRQ	77
static struct msm_spi_platform_data apq8064_qup_spi_gsbi5_pdata = {
#ifdef CONFIG_LGE_BROADCAST_ONESEG_TCC3530
...
static struct spi_board_info spi_broadcast_board_info[] __initdata = {
#ifdef CONFIG_LGE_BROADCAST_ONESEG_TCC3530
	{
		.modalias                    = "isdbt_tcc3530",
		.irq                                 = MSM_GPIO_TO_INT(ONESEG_SPIS_XIRQ),
	...
	{
		.modalias                    = "mb86a35s",
		.irq                                 = MSM_GPIO_TO_INT(ONESEG_INT_N_GPIO),
		.max_speed_hz          = (32*1000*1000),
		.bus_num                = 0,
		.chip_select            = 0,
		.mode                   = SPI_MODE_0,
...
/* LGE_CHANGE_E, For GV Rev.C bring-up, 2012.10.29, jungki.kim[End] */
	...
	if (machine_is_apq8064_cdp())
		...
		printk("[CAM] %s: unmatched machine ID in register_i2c_devices\n", __func__);
 	...
 	if(lge_get_board_revno() >= HW_REV_C){
		if (apq8064_camera_i2c_devices_revC.machs & mach_mask)
			i2c_register_board_info(apq8064_camera_i2c_devices_revC.bus,
				apq8064_camera_i2c_devices_revC.info,
...
static void __init gvdcm_uart_clock_update_rev_a(void)
{
    static char *msm_serial_hsl_4 = "msm_serial_hsl.4";
	static char *msm_serial_hsl_0 = "msm_serial_hsl.0";
 	static char *msm_serial_hsl_3 = "msm_serial_hsl.3";
	...
	for(n=0; n < data->size; n++) {
		...
		/* if UART console enabled */
		if(lge_get_uart_mode() == 1) {
			if( (strcmp(clock_tbl[n].con_id, "core_clk") == 0) &&
				(strcmp(clk->dbg_name, "gsbi4_uart_clk") == 0) &&
				(strcmp(clock_tbl[n].dev_id, "msm_serial_hsl.3") == 0))  /* default .3 */
				clock_tbl[n].dev_id = msm_serial_hsl_4;
			...
			if( (strcmp(clock_tbl[n].con_id, "core_clk") == 0) &&
				(strcmp(clk->dbg_name, "gsbi7_uart_clk") == 0) &&
				(strcmp(clock_tbl[n].dev_id, "msm_serial_hsl.1") == 0))  /* default .1 */
				clock_tbl[n].dev_id = msm_serial_hsl_0;

			if( (strcmp(clock_tbl[n].con_id, "iface_clk") == 0) &&
				(strcmp(clk->dbg_name, "gsbi7_p_clk") == 0) &&
				(strcmp(clock_tbl[n].dev_id, "msm_serial_hsl.1") == 0))  /* default .1 */
				clock_tbl[n].dev_id = msm_serial_hsl_0;
		}else { /* if IrDA enabled */
			if( (strcmp(clock_tbl[n].con_id, "core_clk") == 0) &&
				(strcmp(clk->dbg_name, "gsbi4_uart_clk") == 0) &&
				(strcmp(clock_tbl[n].dev_id, "msm_serial_hsl.3") == 0))  /* default .3 */
				clock_tbl[n].dev_id = msm_serial_hsl_4;
			...
			if( (strcmp(clock_tbl[n].con_id, "core_clk") == 0) &&
				(strcmp(clk->dbg_name, "gsbi7_uart_clk") == 0) &&
				(strcmp(clock_tbl[n].dev_id, "msm_serial_hsl.1") == 0))	/* default .1 */
				clock_tbl[n].dev_id = msm_serial_hsl_3;
...
static void __init gvdcm_uart_clock_update_rev_b(void)
{
	static char *msm_serial_hsl_0 = "msm_serial_hsl.0";
	...
	for(n=0; n < data->size; n++) {
		...
		if( (strcmp(clock_tbl[n].con_id, "core_clk") == 0) &&
			(strcmp(clk->dbg_name, "gsbi4_uart_clk") == 0) &&
			(strcmp(clock_tbl[n].dev_id, "msm_serial_hsl.3") == 0))
			clock_tbl[n].dev_id = msm_serial_hsl_0;
...
static void __init apq8064_common_init(void)
{
	...
	/*
	 * [GV DCM] Change clock source for Rev.A board
	 *
	 */
	if(lge_get_board_revno() == HW_REV_A)
		gvdcm_uart_clock_update_rev_a();
	else if(lge_get_board_revno() >= HW_REV_B && lge_get_uart_mode())
		gvdcm_uart_clock_update_rev_b();
...
/* ehee.lee@lge.com [START] for NFC */
	...
	if (machine_is_apq8064_mtp() || machine_is_apq8064_gk()) {
		msm_hsic_pdata.log2_irq_thresh = 4;
...
static void __init apq8064_cdp_init(void)
{
	...
	if(lge_get_board_revno() == HW_REV_A) {
		/*  GSBI7 - Rev.A   (without irda & without irrc)
		 *  ttyHSL0(GSBI7) - UART Console without IrDA
		 */
		if (lge_get_uart_mode()) { /* ttyHSL0 */
			apq8064_device_uart_gsbi7.id = 0;
			platform_device_register(&apq8064_device_uart_gsbi7); /* for UART console & IrDA Uart */
		}
		...
		if (lge_get_uart_mode()) {
			platform_device_register(&apq8064_device_uart_gsbi4); /* for Uart Console */
		} else {
#if defined(CONFIG_LGE_IRDA)
			apq8064_irda_pdata.uart_rx_gpio = 11;
			apq8064_irda_pdata.uart_tx_gpio = 10;
			apq8064_device_uart_gsbi4.dev.platform_data = &apq8064_irda_pdata;
#endif
			apq8064_device_uart_gsbi4.id = 3;
			platform_device_register(&apq8064_device_uart_gsbi4); /* for IrDA Uart */
		}
	}
#if defined(CONFIG_LGE_BROADCAST_ONESEG)
	apq8064_device_qup_spi_gsbi5.dev.platform_data =
...
#if defined(CONFIG_LGE_FELICA) || defined(CONFIG_LGE_NFC_SONY_CXD2235AGG)
	/*
		GV FeliCa/NFC GSBI Configuration
		Rev.A,B GSBI6
		Rev.C   GSBI3
	 */
	if(lge_get_board_revno() < HW_REV_C){
		...
		platform_add_devices(felica_uart_gsbi3_devices, ARRAY_SIZE(felica_uart_gsbi3_devices));
		pr_info("[FELICA-DEBUG] apq8064_device_uart_gsbi3.id : %d",  apq8064_device_felica_gsbi3.id);
	}
#endif
#if defined(CONFIG_LGE_IRRC)
	if(lge_get_board_revno() > HW_REV_A){
...
MACHINE_START(APQ8064_GK, "QCT APQ8064 GK")
