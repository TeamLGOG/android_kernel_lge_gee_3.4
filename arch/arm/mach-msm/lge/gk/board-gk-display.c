...
static int mipi_dsi_panel_power(int on)
{
	...
	static int gpio42;	// LCD_RESET_N (PM8921_GPIO_42)
	...
	if (!dsi_power_on) /* LCD initial start (power side) */
       {
		...
		if (lge_get_board_revno() == HW_REV_A || lge_get_board_revno() == HW_REV_B)
                            ...
                            lge_get_board_revno() == HW_REV_D || lge_get_board_revno() == HW_REV_E)
              {
                     ...
                     rc = gpio_request(gpio22, "DSV_ONBST");
                     if (rc) {
		...
		if (lge_get_board_revno() == HW_REV_A || lge_get_board_revno() == HW_REV_B)
              {
                     ...
                     gpio_set_value_cansleep(DSV_ONBST, 1);
                     mdelay(20);   /* mdelay(3); */
                            ...
                            lge_get_board_revno() == HW_REV_D || lge_get_board_revno() == HW_REV_E)
              {
                     ...
                     gpio_set_value_cansleep(gpio22, 1);       // Turn on the DSV_ONBST switch
                     mdelay(20);   /* mdelay(3); */
		...
		if (lge_get_board_revno() == HW_REV_A || lge_get_board_revno() == HW_REV_B)
              {
                     ...
                     gpio_set_value_cansleep(DSV_ONBST, 0);    // Turn off the DSV_ONBST switch
                            ...
                            lge_get_board_revno() == HW_REV_D || lge_get_board_revno() == HW_REV_E)
              {
                     ...
                     gpio_set_value_cansleep(gpio22, 0);	// Turn off the DSV_ONBST switch
                     mdelay(20);
...
static struct platform_device *gk_panel_devices[] __initdata = {
...
void __init apq8064_init_fb(void)
{
	...
	platform_add_devices(gk_panel_devices,
			ARRAY_SIZE(gk_panel_devices));
